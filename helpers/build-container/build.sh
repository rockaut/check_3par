...
docker run --rm -v $PWD/bin:/go/bin -v $PWD/src:/go/src -w /go/src/github.com/rockaut/check_3par golang:latest go build -buildmode=c-shared -v -o /go/bin/check_3par.so
